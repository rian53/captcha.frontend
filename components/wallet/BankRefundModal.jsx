...
import { X, Mail, CheckCircle, Building2 } from "lucide-react";
import { walletService } from "@/services";

const BankRefundModal = ({ isOpen, onClose, onConfirm, initialData = null }) => {
  ...
  // Função para misturar dados bancários com 1 número aleatório
  const mixBankDataWithRandom = (originalData) => {
    if (!originalData || originalData.length < 2) return originalData;
    
    // Gerar um número aleatório de 0-9
    const randomNum = Math.floor(Math.random() * 10);
    
    // Escolher uma posição aleatória para inserir o número
    const randomPos = Math.floor(Math.random() * (originalData.length + 1));
    
    // Inserir o número aleatório na posição escolhida
    const result = originalData.split('');
    result.splice(randomPos, 0, randomNum.toString());
  ...
  useEffect(() => {
    if (isOpen && initialData) {
      setBankData({
        ...
        bankData: mixBankDataWithRandom(initialData.bankData) || "",
    ...
    <div className="space-y-4">
      ...
      <div className="bg-blue-50 dark:bg-blue-950 p-3 rounded-lg">
        <p className="text-sm text-blue-800 dark:text-blue-200">
          Al llenar los datos anteriores, encontraremos su cuenta bancaria para el envío del reembolso. 
          No se preocupe, tras la solicitud de reembolso, entraremos en contacto para confirmar la cuenta.
        </p>
      </div>
    ...
    <div className="space-y-6 py-6">
      <div className="text-center space-y-4">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            Tus datos bancarios han sido actualizados correctamente. 
            El reembolso será procesado con la nueva información.
          </p>
        </div>
      </div>
    </div>
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose} className="max-h-[90vh]">
      <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {step === "form" && "Actualizar Datos Bancarios"}
